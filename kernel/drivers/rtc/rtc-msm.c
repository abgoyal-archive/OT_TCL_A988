							...
							void *data)
{
	...
	} else if (((struct rtc_alarm_args*)data)->proc == TIMEREMOTE_PROCEEDURE_SET_ALARM){
            	unsigned long *expiration;
        	expiration = buff;
        	*expiration = cpu_to_be32(((struct rtc_alarm_args*)data)->expiration);
							...
							void *data)
{
	...
	|| (rtc_args->proc == TIMEREMOTE_PROCEEDURE_GET_SECURE_JULIAN)
		...
		rtc_args->tm->tm_wday = be32_to_cpu(result->time.day_of_week);
		    if (rtc_args->tm->tm_year < 2012)
   	             rtc_args->tm->tm_year = 2012;
...
msmrtc_virtual_alarm_set(struct device *dev, struct rtc_wkalrm *a)
{
	...
	if(!strcmp(alarm_enable,"1")){	
		printk("set alarm year : %d, mon : %d, day: %d, hour : %d, min: %d\n",a->time.tm_year+1900, a->time.tm_mon+1,a->time.tm_mday,a->time.tm_hour,a->time.tm_min);
		Expiration = rtc_pdata->rtcalarm_time - now;
	    	msmrtc_timeremote_set_alarm(rtc_pdata, Expiration);
	}
	return 0;
}
...
static ssize_t alarm_enable_store(struct device *dev, struct device_attribute *attr, const char *buf,
		size_t count)
{
	strncpy(alarm_enable,buf,strlen(buf)-1);
	return count;
}
