...
static long taos_ioctl( struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	//u8 reg_val_temp = 0;
	//taos_datap = container_of(inode->i_cdev, struct taos_data, cdev);
printk("XXXXXXXXXXXXXXXX taos_ioctl XXXXXXXXXXXXXX\n");
...
static int taos_get_lux(void)
{
	...
	//printk("Reg0x14=%d,Reg0x15=%d,ch0=%d\n", chdata[0],chdata[1],chdata[0] + chdata[1] * 256);
	//printk("Reg0x16=%d,Reg0x17=%d,ch1=%d\n", chdata[2],chdata[3],chdata[2] + chdata[3] * 256);
      /* fellow about lux alg modification,is according to taos and hardware test*/
	...
	if (raw_ir > raw_clear) {
	     lux=lux_history[0];//xqj,keep unchange,same as the above
	     return lux;
	}
	...
	if (dev_gain == 0 || dev_gain > 127) {
       ...
       clear_nf =raw_clear*1000;
	clear_nf=(clear_nf+(dev_gain>>1))/dev_gain;
	//printk("1.clear_nf =%d\n",clear_nf);
	clear_nf=(clear_nf*400+(taos_cfgp->als_time>>1))/taos_cfgp->als_time;
	//printk("2.clear_nf =%d\n",clear_nf);

	ir_nf =raw_ir*1000;
	ir_nf=(ir_nf+(dev_gain>>1))/dev_gain;
	//printk("1.ir_nf =%d\n",ir_nf);
	ir_nf=(ir_nf*400+(taos_cfgp->als_time>>1))/taos_cfgp->als_time;
	//printk("2.ir_nf =%d\n",ir_nf);
       ratio=(1000*ir_nf)/clear_nf;
   	//printk("1.ratio =%d\n",ratio);
       if(ratio>540)
	      lux=0;
      else if(ratio<=540 && ratio >400)
      	{
	  	clear_nf= (clear_nf*100)/18;
		ir_nf=(ir_nf*100)/37;
		lux=2*(Sf)*(13*clear_nf-24*ir_nf)/100;
      	}
      else if(ratio<=400 && ratio >120)
      	{
	  	clear_nf= (clear_nf*100)/13;
		ir_nf=(ir_nf*100)/22;
		lux=2*(Sf)*(13*clear_nf-24*ir_nf)/100; 
      	}
      else if(ratio<=120)
      	{
	  	clear_nf= (clear_nf*100)/7;
		ir_nf=(ir_nf*100)/10;
		lux=2*(Sf)*(13*clear_nf-24*ir_nf)/100;  	
      	} 	
	   //	printk("ffff  lux =%d\n",lux);
...
Ratio=0.4 to= 0.54   CH0_NF= CH0_NF/0.18,CH1_NF=CH1_NF/0.37, lux=2*(Sf)*(0.13*CH0_NF-0.24*CH1_NF)

Ratio=0.12 to 0.4    CH0_NF= CH0_NF/0.13,CH1=CH1_NF/0.22,    lux=2*(Sf)*(0.13*CH0_NF-0.24*CH1_NF)

Ratio<0.12           CH0_NF= CH0_NF/0.07,CH1=CH1_NF/0.1,     lux=2*(Sf)*(0.13*CH0_NF-0.24*CH1_NF)
*/
	/*
...
static void taos_prox_poll_timer_func(unsigned long param)
{
	...
	taos_prox_poll_timer_start(200);//2012.1.6  modified  by xqj,poll time change from 400 to 200ms
}
...
static void taos_als_poll_timer_func(unsigned long param)
{
	...
	taos_als_poll_timer_start(250);//restore to 250 ms,als only has little consumpition
}
