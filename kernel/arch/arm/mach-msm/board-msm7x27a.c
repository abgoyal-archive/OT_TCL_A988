...
enum {
		...
		GPIO_DISPLAY_PWR_EN = 90, // GPIO used in V2
...
static unsigned mipi_dsi_gpio[] = {
	...
	GPIO_CFG(GPIO_DISPLAY_PWR_EN, 0, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL,
		GPIO_CFG_2MA),       /* GPIO_DISPLAY_PWR_EN */
};
...
#define WLAN_USING_LDO
#ifdef WLAN_USING_LDO
#define GPIO_WLAN_CHIP_3300MV 119
static int bt_is_using = false;
static int wlan_is_using = false;
#endif
...
fail_power:
	...
	if (on) {
		if (!wlan_is_using)
		{
			gpio_set_value(GPIO_WLAN_CHIP_3300MV, 1);
		}
		bt_is_using = true;
	}else {
		if (!wlan_is_using)
		{
			gpio_set_value(GPIO_WLAN_CHIP_3300MV, 0);
		}	
		bt_is_using = false;
	}
#endif
...
{
	return sprintf(buf,
                 __stringify(EV_KEY) ":" __stringify(KEY_MENU)  ":52:842:104:44"
            ":" __stringify(EV_KEY) ":" __stringify(KEY_HOME)   ":177:842:104:44"
            ":" __stringify(EV_KEY) ":" __stringify(KEY_SEARCH) ":300:842:104:44"
            ":" __stringify(EV_KEY) ":" __stringify(KEY_BACK)   ":427:842:104:44"
            "\n");
}
...
/**
 * Bug#212873
 * In FTM mode USB device will refresh and communication is not stable
 * Leave only PID 0x9018 for MMITEST, the bug does not reproducted
 */
...
#endif  /* #ifndef TARGET_BUILD_MMITEST */
...
static struct android_usb_product usb_products[] = {
	{ /* switch 9018 to the first since it's default device for adb enabled */
		.product_id	= 0x9018,
...
static struct usb_mass_storage_platform_data mass_storage_pdata = {
	.nluns		= 1, /* Change the number of the mass_storage disk for Bug #206540 [Alvin 2012.02.20] */
	.vendor		= "Qualcomm Incorporated",
	.product	= "Mass storage",
	.release	= 0x0100,
	.cdrom_num	= -1, /* No cdrom for Bug#206540 [Alvin 2012.02.20] */
};
...
/**
 * Bug#212873
 * For MMITEST we usb PID 0x9018
 * Commonly 0x9017 is default
 */
#ifndef TARGET_BUILD_MMITEST
	.product_id	= 0x9017,
#else
...
static struct msm_gpio sdc3_cfg_data[] = {
	...
	{GPIO_CFG(21, 3, GPIO_CFG_OUTPUT, GPIO_CFG_PULL_UP, GPIO_CFG_10MA),
								"sdc3_dat_5"},
	{GPIO_CFG(107/*108*/, 3, GPIO_CFG_OUTPUT, GPIO_CFG_PULL_UP, GPIO_CFG_10MA),
								"sdc3_dat_4"},
#endif
};
#endif
...
static struct msm_gpio sdc4_cfg_data[] = {
	...
	{GPIO_CFG(107, 1, GPIO_CFG_OUTPUT, GPIO_CFG_PULL_UP, GPIO_CFG_10MA),
								"sdc4_cmd"},
	{GPIO_CFG(107/*108*/, 1, GPIO_CFG_OUTPUT, GPIO_CFG_PULL_UP, GPIO_CFG_10MA),
								"sdc4_dat_0"},
...
static unsigned int msm7x2xa_sdcc_slot_status(struct device *dev)
{
	...
	//if((machine_is_msm7x27a_sku3())||(machine_is_msm7x27a_sku1()&&hw_version_is(2,0))){
		status = !status;
	//}
	return status;
}
...
static struct android_pmem_platform_data android_pmem_adsp_pdata = {
	...
	.cached = 0,//zhangzhenjun for bug 222643
...
{
	boot_splash = 0;   //simple_strtol(str, NULL, 0);
...
static void lcd_camera_power_init(void)
{
  ...
  if (!boot_splash) {
	...
	rc = gpio_request(GPIO_DISPLAY_PWR_EN, "truly_panel_power_2V8");
	if (rc < 0) {
		pr_err("failed to request truly_panel_power_2V8 \n");		
	}
	rc = gpio_tlmm_config(mipi_dsi_gpio[1], GPIO_CFG_ENABLE);
	if (rc) {
		pr_err("Failed to enable truly_panel_power_2V8 controller! \n");
		gpio_free(GPIO_DISPLAY_PWR_EN);		
	}
	gpio_direction_output(GPIO_DISPLAY_PWR_EN, 1);
	printk("======> %s called to enable GPIO_90 to open MIPI LCD 2.85V power. \n", __func__);
  return;
...
static void lcdc_truly_gpio_init(void)
{
	...
	{
		if (gpio_request(lcdc_gpio_table[i], lcdc_gpio_name_table[i]))
		{
...
static void config_lcdc_gpio_table(uint32_t *table, int len, unsigned enable)
{
	...
	if (lcdc_gpio_initialized) {
#ifndef CONFIG_MACH_MSM7X27A_SKU3 //SKU3 LCDC

		for (n = 0; n < len; n++)
		{
			rc = gpio_tlmm_config(GPIO_CFG(table[n], 0, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA), GPIO_CFG_ENABLE);
			if (rc < 0)
				pr_err("Error config lcdc gpio:%d\n", table[n]);
		}
		//reconfig PCLK to 4ma
		rc = gpio_tlmm_config(GPIO_CFG(GPIO_LCDC_PCLK, 1, GPIO_CFG_OUTPUT, GPIO_CFG_PULL_DOWN, GPIO_CFG_4MA), GPIO_CFG_ENABLE);
		if (rc < 0)
			printk("Error config PCLK\n");
...
{
	config_lcdc_gpio_table(lcdc_gpio_table, ARRAY_SIZE(lcdc_gpio_table), enable);
...
static int msm_fb_detect_panel(const char *name)
{
	...
	if (machine_is_msm7x27a_surf()){
		...
		if (!strncmp(name, "lcdc_truly_hvga_ips3p2335_pt", 28))
			ret = 0;
		else if (!strncmp(name, "mipi_video_hx8363a_wvga", 23))
			ret = 0;
	}else {
		ret = -ENODEV;
	}
...
int mipi_set_bl(int level)
{
	...
	if (!dsi_lcd_bl_initialized) {
		pmapp_disp_backlight_init();
		dsi_lcd_bl_initialized = 1;
	}
	if(fastboot_power_off == 3)
	{
		printk("LYF_DEBUG mipi_set_bl fastboot_power_off %d level %d\n",fastboot_power_off,level);
		if(level != 0)
			return 0;
	}
	...
	{
		printk("LYF_DEBUG mipi_set_bl fastboot_power_off %d level %d\n",fastboot_power_off,level);
		printk("LYF_DEBUG mipi_set_bl fastboot_bklight_off %d level %d\n",fastboot_bklight_off,level);
...
static struct platform_device mipi_dsi_panel_device = {
	...
	.name = "mipi_truly",
#elif defined(CONFIG_FB_MSM_MIPI_DSI_HX8363A)         /* will be added to configuration file later by wangjin  */
	.name = "mipi_hx8363a",                /* product.  */
#else
	.name = "mipi_unknown",
#endif
	.id = 0,
...
#if 1//comment by Yanjingbin.  for ov5640 use it ndef CONFIG_MACH_MSM7X27A_SKU1 //SKU1&SKU3 don't use this
...
static void msm_camera_af_vreg_config(int vreg_en)
{
	int rc;

	if (vreg_gp3 == NULL) {
		vreg_gp3 = vreg_get(NULL, "emmc");
		if (IS_ERR(vreg_gp3)) {
			pr_err("%s: vreg_get(%s) failed (%ld)\n",
...
#ifdef CONFIG_MACH_MSM7X27A_SKU3
#define GPIO_SKU1_CAM_VGA_SHDN 118//91
...
#if 0  //not used for camera in mojito board
...
static struct msm_gpio uart2dm_gpios[] = {
	...
	{GPIO_CFG(21, 2, GPIO_CFG_INPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA),
							"uart2dm_rx"    },
	{GPIO_CFG(107/*108*/, 2, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA),
							"uart2dm_tx"    },
};
...
//#define GPIO_WLAN_3V3_EN machine_is_msm7x27a_sku1()?119:92
//#define USING_3300MV_PATCH

#ifndef USING_3300MV_PATCH
...
int wlan_power_ctrl(unsigned char on)
{
	...
	if(on){
		...
		if(machine_is_msm7x27a_sku3())
		{
			...
			wlan_is_using = true;
#else
			rc = vreg_set_level(bt_vregs[1].vregs,
					bt_vregs[1].level);
			...
			rc = vreg_enable(bt_vregs[1].vregs);
...
//ZhiShen 2011-11-08 remove for mojito_qrd_pre_cs
//#define GPIO_LCDC_BRDG_PD	128
...
static int msm_fb_dsi_client_reset_sku3(void)
{
	int rc = 0;
...
#if 0   // remove reset steps into mipi_hx8363a driver.
...
static int msm_fb_dsi_client_reset(void)
{
	...
	//} else if (machine_is_msm7x27a_sku3()) {
		rc = msm_fb_dsi_client_reset_sku3();
	//}
...
static int mipi_dsi_panel_power_sku3(int on)
{
	...
	if(on)
	{
		gpio_set_value(GPIO_DISPLAY_PWR_EN, 1);	
	}
	else
	{
		gpio_set_value(GPIO_DISPLAY_PWR_EN, 0);	
	}
	...
	if (on) 
	{
        //TCTNB.#193383 Dont light up too early.
		//rc = pmapp_disp_backlight_set_brightness(36);		
		if (rc)
			pr_err("====> backlight set brightness failed\n");
	} 
	else 
...
static int mipi_dsi_panel_power(int on)
{
	int rc = 0;
        ...
        rc = mipi_dsi_panel_power_sku3(on);
...
#ifdef WLAN_USING_LDO
	status = gpio_tlmm_config(GPIO_CFG(GPIO_WLAN_CHIP_3300MV, 0, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA),GPIO_CFG_ENABLE);
	if(status){
                pr_err("%s gpio_tlmm_config 119 failed,error = %d\n", __func__,status);
        }
	gpio_set_value(GPIO_WLAN_CHIP_3300MV, 0);
...
/* Comment by yanjingbin for upgrade 10982302
#ifdef CONFIG_OV5640
		msm_camera_sensor_ov5640_data.sensor_reset = GPIO_SKU1_CAM_5MP_CAMIF_RESET_N;
#endif
...
//wqf
	 ...
	 //printk("wqf-----------------board config i2c gpio\n");
#endif
#endif
