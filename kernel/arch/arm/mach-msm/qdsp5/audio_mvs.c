 ...
 */
/*==============================================*/
/* date       |  author       |  Bug ID   |  module | communication */
/* 2012-02-09 |  ZhangLong    |  209166   |         | Move the init functions from audio_mvs_open to audio_mvs_init to avoid crash */
/* modify accroding to https://www.codeaurora.org/gitweb/quic/qrd-android/?p=kernel/msm.git;a=commit;h=0b3ce40709ddf86d084eb6e5bd8da4d53e89efc8     */                                                  
/*==============================================*/
...
static int audio_mvs_open(struct inode *inode, struct file *file)
{
    ...
    /*ZhangLong 2012-02-09 modify for bug 209166 begin*/
    ...
    */
	...
	if (audio_mvs_info.state == AUDIO_MVS_CLOSED) { */
    /*ZhangLong 2012-02-09 modify for bug 209166 end*/
...
{
    /*ZhangLong 2012-02-09 add for bug 209166 begin*/
    memset(&audio_mvs_info, 0, sizeof(audio_mvs_info));
    mutex_init(&audio_mvs_info.lock);
    mutex_init(&audio_mvs_info.in_lock);
    mutex_init(&audio_mvs_info.out_lock);
    ...
    wake_lock_init(&audio_mvs_info.idle_lock,
                   WAKE_LOCK_IDLE,
                   "audio_mvs_idle");
    /*ZhangLong 2012-02-09 add for bug 209166 end*/
