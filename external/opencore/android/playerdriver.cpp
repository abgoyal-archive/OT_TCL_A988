        ...
        public PVErrorEventObserver
{
  public:
    ...
    ~PlayerDriver();
    void liveNoSeekPlay(); //zhangzhenjun for bug  250632
...
/*zhangzhenjun for bug  250632
** when opencore play the live rtsp, it can not seek, but 
** the resume will call seek function, then it can play the stream,
** so add this function to play the live rtsp when suspend and resume
*/
void PlayerDriver::liveNoSeekPlay(){
    if (mIsLiveStreaming) {
...
void PlayerDriver::handleSetDataSource(PlayerSetDataSource* command)
{
    ...
    // begin 211412 add SDP support by Hanling.Zhang @ 20120211
    //add by lrc for rtsp/sdp 36227 begin
    ...
    //add by lrc for rtsp/sdp 36227 end
    // end 211412 add SDP support by Hanling.Zhang @ 20120211
    ...
    if (strncmp(url, "rtsp:", strlen("rtsp:")) == 0) {
	    ...
	    // begin 211412 modified for SDP support by Hanling.Zhang @ 20120211
	    	//modify by lrc for rtsp/sdp 36227 and 35089 begin	
		if (strncmp(url, "sharedfd:", strlen("sharedfd:")) == 0)
		{
		    if(!IsSdpAttachment)
		    {
		        //LOGE("------set common source to PV player successfully----");
		        mDataSource->SetDataSourceFormatType((const char*)PVMF_MIME_FORMAT_UNKNOWN); // Let PV figure it out			
		    }else{
		        //LOGE("--- set SDP source to PV player successfully---");
		...
		else if(strrchr(url, '.'))
		{
	     ...
	     //modify by lrc for rtsp/sdp 36227 and 35089 end
		 // end 211412 modified for SDP support by Hanling.Zhang @ 20120211
    }
...
status_t doUsePVPlayer(const char *filename)
{
	...
	// begin 211412 add SDP support by Hanling.Zhang @ 20120211
	//add by baisheng.zhao for CR support sdp
    if(strrchr(filename, '.'))
       {
